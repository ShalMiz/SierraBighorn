###############################################################################################
# Combining downscaled categorical and regression images
###############################################################################################
...
local.classification.path <- "~/SierraBighorn/downscaledv5_localForests/downscaled_classification/downscaled/"
local.classification.names <- list.files(local.classification.path)

#local regression downscaled 
local.regression.path <- "~/SierraBighorn/downscaledv5_localForests/downscaled_regression/downscaled/"
local.regression.names <- list.files(local.regression.path)

#local downscales combined 
local.combined.path <- "~/SierraBighorn/downscaledv5_localForests/downscaled_combined/downscaled/"
...
for (day in 1:n) {
  ...
  writeRaster(local.classfication, filename=paste0(local.combined.path,"downscaled.combined.", format(lst.date[day],"%Y%m%d"),".tif"),     format="GTiff",option="COMPRESS=LZW",datatype="INT1U",overwrite=TRUE)
