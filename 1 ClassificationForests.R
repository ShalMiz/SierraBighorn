###############################################################################################
# Local Classification Random Forest Training Script
###############################################################################################
...
Rdata_path <- "~/SierraBighorn/Rdata/localForests/classification_forest/"
...
# 250 is an upper bound for the number of local forests (there were 221 classf. RFs)
...
## Partitioning sequence of Landsat into 450*450 block and get their start row and column index
rowBeg <- seq(top.left.row, bottom.right.row, block_size)
rowBeg <- rowBeg[-length(rowBeg)] # remove last item of this vector
colBeg <- seq(bottom.left.col, nrows, block_size)
colBeg <- colBeg[-length(colBeg)] # remove last item of this vector
...
for (i in rowBeg) {
  ...
  for (j in colBeg) {
    ...
    # if there's only one class of data or if there is less than 1000 datapoints to train on then we do not train that block
    if (length(levels(train.dat$class))==1 || dim(train.dat)[1]<1000){
      next
    }
    
    print(paste0("Training Started: ",Sys.time()))
    ranger.classifier <- ranger(class~ elev+ slope + mod + asp + windspeed + nw.barrierdist+ sw.barrierdist+ w.barrierdist+ sw.waterdist+ w.waterdist + da, data=train.dat,num.trees=100,probability=TRUE, importance = 'impurity')
    print(paste0("Training Ended: ",Sys.time()))            
    
    var.importance[k,1] <- i
    var.importance[k,2] <- j
    var.importance[k,3:13] <- ranger.classifier$variable.importance
    
    save(ranger.classifier,file=paste0(Rdata_path,"forest/","ranger.classifier.",i,".",j,".Rda"))
    rm(ranger.classifier)
    
    rm(train.dat, train.da.nNA)
    save(var.importance, file = paste0(Rdata_path,"variable.importance.RData"))
    save(train.idx,file=paste0(Rdata_path,"train_indices_locations/","train.indices.",i,".",j,".RData"))
    rm(train.idx)
    k <- k+1
